...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("distribution_id", type=int, help="ID of the mailing to send")

    def handle(self, *args, **options):
        distribution_id = options["distribution_id"]
        try:
            distribution = Distribution.objects.get(pk=distribution_id)
            send_distribution_task.delay(distribution.pk)  # Запускаем задачу Celery асинхронно
            self.stdout.write(
